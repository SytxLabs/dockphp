...
/** Contract used by Resources to talk to the Docker Engine API. Allows tests to substitute a fake transport with no real socket. */
interface DockerTransportInterface
{
    ...
    /**
     * Like request(), but sends $rawBody verbatim (no JSON encoding). Used for endpoints that take a tar stream (e.g. copying files into a container).
     *
    ...
    /**
     * Streams the response body to $onChunk as it arrives instead of buffering it. Used for `follow`ed logs, streamed stats, and `/events`. Returning false from $onChunk stops the transfer early without throwing.
     *
    ...
    /**
     * Combination of requestRaw() and stream(): raw request body, streamed response. Used only for `docker build`.
     *
     * @param array<string, mixed> $query
     * @param callable(string): (bool|void) $onChunk
    ...
    /** Resolves the API version prefix used for requests (e.g. "1.43"), either the manually configured override or the version detected lazily via a one-time `/version` call. */
