...
/**
 * Sends requests to the Docker Engine API using Guzzle, either over a Unix domain socket (default) or a TCP host (optionally with TLS). No shell commands, no Docker CLI.
 *
 * For the Unix socket case, the internal base URL "http://localhost" is never actually resolved over the network
 * the CURLOPT_UNIX_SOCKET_PATH option (passed through Guzzle's "curl" client config) routes the connection through the socket instead, the host part only satisfies HTTP syntax.
 *
...
class DockerTransport implements DockerTransportInterface
{
    ...
    public function __construct(private readonly ?string $socketPath, ?string $apiVersion = null, private readonly float $connectTimeout = 5.0, private readonly float $timeout = 30.0, private readonly ?string $host = null, private readonly int $port = 2375, private readonly bool $tls = false, private readonly ?string $caFile = null, private readonly ?string $certFile = null, private readonly ?string $keyFile = null)
    {
     ...
     */
    public static function forTcp(string $host, int $port = 2375, bool $tls = false, ?string $caFile = null, ?string $certFile = null, ?string $keyFile = null, ?string $apiVersion = null, float $connectTimeout = 5.0, float $timeout = 30.0): static
    {
    ...
    {
        if ($this->resolvedApiVersion !== null) {
            return $this->resolvedApiVersion;
        }
        $data = $this->bufferedRequest('GET', '/version', null, [], [])->json();
        ...
        return $this->resolvedApiVersion = $data['ApiVersion'];
    }

    private function buildClient(): Client
    {
        $config = ['http_errors' => false, 'connect_timeout' => $this->connectTimeout, 'timeout' => $this->timeout];
    ...
    {
        return $body === null ? [null, []] : [$this->encodeJson($body), ['Content-Type: application/json']];
    }
    ...
    private function streamedRequest(string $method, string $path, ?string $payload, array $headerLines, array $query, callable $onChunk): int
    {
        ...
        try {
            ...
            if ($e instanceof RequestException && $e->hasResponse()) {
                ...
                if ((int) ($context['errno'] ?? 0) === CURLE_WRITE_ERROR && $sink->isAborted()) {
    ...
    {
        $target = $this->socketPath ?? (($this->tls ? 'tcps://' : 'tcp://') . $this->host . ':' . $this->port);
        if (!($e instanceof RequestException)) {
            return DockerConnectionException::fromGuzzleError($target, $e->getMessage(), $e);
        }
        $context = $e->getHandlerContext();
        return DockerConnectionException::fromCurlError($target, (string) ($context['error'] ?? $e->getMessage()), (int) ($context['errno'] ?? 0));
    }
    ...
    protected function buildUrl(string $path, array $query): string
    {
        ...
        return ($this->socketPath !== null ? 'http://localhost' : ($this->tls ? 'https' : 'http') . '://' . $this->host . ':' . $this->port) . $path . ($queryString !== '' ? '?' . $queryString : '');
    }

    /**
     * Docker's query parameters use JSON encoding for array values (e.g. `filters`) and lowercase "true"/"false" for booleans.
     *
     * @param array<string, mixed> $query
     */
