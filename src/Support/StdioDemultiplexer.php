...
/**
 * Splits Docker's multiplexed stdout/stderr stream format intoindividual frames.
 * Applies only to containers created without a TTY (`Tty: false`) with a TTY, Docker sends raw bytes and no demuxing is needed or possible.
 *
 * Frame format: 1 byte stream type (0 = stdin, 1 = stdout, 2 = stderr), 3 reserved bytes, 4-byte big-endian payload length, then the payload.
 *
...
final class StdioDemultiplexer
{
    ...
    /**
     * Feeds a streamed chunk in; emits every complete frame found so far via `$onFrame(string $stream, string $payload)`.
     * Incomplete trailing frames are kept buffered until the next call.
     *
