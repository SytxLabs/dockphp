...
{
    /**
     * Creates an exec instance for a running container. Returns the new exec instance's Id (via `$response->json()['Id']`) it must be started with start()/startStream() to actually run.
     *
    ...
    /**
     * Starts (runs) a previously created exec instance and returns its full, non-streamed output.
     * Unless the exec was created with Tty=true, the output is multiplexed - see {@see \Sytxlabs\Dockphp\Support\StdioDemultiplexer}.
     *
    ...
    /**
     * Like start(), but streams output to $onChunk as it's produced. Return false from $onChunk to stop reading early.
     *
    ...
    {
        return $this->transport->request('POST', "/exec/{$execId}/resize", null, ['h' => $height, 'w' => $width]);
    }
