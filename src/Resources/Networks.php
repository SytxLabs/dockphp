...
final class Networks extends AbstractResource
{
    ...
    {
        $response = $this->transport->request('GET', '/networks', null, ['filters' => $filters === [] ? null : $filters]);
        return array_map(static fn(array $item): NetworkInfo => NetworkInfo::fromArray($item), $this->decodeList($response));
    }

    public function inspect(string $id, bool $verbose = false, ?string $scope = null): NetworkInfo
    {
        $response = $this->transport->request('GET', "/networks/{$id}", null, ['verbose' => $verbose, 'scope' => $scope]);
    ...
    {
        return $this->transport->request('POST', "/networks/{$id}/disconnect", ['Container' => $containerId, 'Force' => $force]);
    }
    ...
    {
        return $this->transport->request('POST', '/networks/prune', null, ['filters' => $filters === [] ? null : $filters]);
    }
}
